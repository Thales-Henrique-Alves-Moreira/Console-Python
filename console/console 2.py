# This is the executer of the scripts (plugins).
# In future versions it will be using threads.
...
class Console():
    ...
    def readFunctions(self):
        ...
        for path in paths:
            with open(curPath + "/plugins/" + path + "/functions.json", "r") as ffjson:
                if not os.path.exists(curPath + "/plugins/" + path + "/not-finished.txt"):
                    self.funcsjson.append(json.load(ffjson))

        with open(curPath + "/config/functions.json", "r") as ffjson:
            self.funcsjson.append(json.load(ffjson))
